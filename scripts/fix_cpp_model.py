...
8. WRONG SEMANTICS: `x86_movd_movq_to_xmm` writes only `operand_size` (4 or 8)
   bytes of the destination XMM register, merging the rest — but MOVD/MOVQ to
   an XMM register zero-extend across the full register (SDM: "the destination
   register is zero-extended"). Confirmed against Bochs and real hardware.
   -> write 16 bytes; the value is already the zero-extended unsigned source.
   See docs/backend-differences.md §4d.

9. WRONG INSTRUCTION: the two-byte dispatch routes 0F 12 with the F3 and F2
   mandatory prefixes — MOVSLDUP and MOVDDUP, which x86isa does not implement —
   to `x86_movlps_movlpd_op_en_rm`, silently executing MOVLPS/MOVLPD semantics
   instead (the aliased load leaves the destination's high half stale). The
   neighbouring unimplemented prefix arms (F3 0F 16 MOVSHDUP, F3 0F 7E MOVQ)
   correctly raise the model's "Opcode Unimplemented" error, which the shim
   classifies as a skippable backend gap. -> make the two aliased arms raise
   the same error. See docs/backend-differences.md §4e.
"""
import os
import re
import sys
...
# the host's line ending, so running this script under Windows would rewrite
# all 280k lines of the model with CRLF — thousands of lines of diff around a
# handful of patched ones, flipped back again by the next run on Linux. The
# vendored artifact is LF and stays LF. Reads need no such flag: universal
# newlines already normalise CRLF to "\n" everywhere, so re-running on a file
# someone else converted is safe.
...
def refcount_scratch(cpp_path: str) -> None:
    ...
    runs_wrapped = 0
    already = False
    with open(cpp_path) as src, open(tmp_path, "w", newline="\n") as dst:
        pending = []
...
MARK_MOVD = "/* oracle-fix-8: zero-extend to the full register */"


def fix_movd_zero_extend(cpp_path: str) -> None:
    """MOVD/MOVQ to an XMM register zero-extend; the model merged."""
    src = open(cpp_path).read()
    if MARK_MOVD in src:
        print(f"{cpp_path}: movd zero-extend fix already applied, skipped")
        return
    start, end = function_body(src, "zx86_movd_movq_to_xmm", "fix 8")
    body = src[start:end]
    calls = list(re.finditer(r"zwrite_xmmi_sizze\((z\w+), ", body))
    if len(calls) != 1:
        raise SystemExit(
            f"{cpp_path}: expected 1 write_xmmi_size call in x86_movd_movq_to_xmm, found "
            f"{len(calls)} — model layout changed, fix 8 needs review"
        )
    m = calls[0]
    body = body[: m.start()] + f"zwrite_xmmi_sizze({MARK_MOVD} UINT64_C(0x10), " + body[m.end() :]
    with open(cpp_path, "w", newline="\n") as f:
        f.write(src[:start] + body + src[end:])
    print(f"{cpp_path}: movd/movq-to-xmm now writes the full 16 bytes")


# ------------------------------------------------------------------- fix 9

MARK_ALIAS = "/* oracle-fix-9: unimplemented, not movlps */"
UNIMPL_CALL = 'zx86_model_errorzIuzK("Opcode Unimplemented in x86isa!")'
PREFIX_CMP = re.compile(r"== UINT64_C\(0x([0-9A-Fa-f]{2})\)\);")


def fix_dup_aliasing(cpp_path: str) -> None:
    """F3/F2 0F 12 (MOVSLDUP/MOVDDUP) must not execute as MOVLPS/MOVLPD."""
    src = open(cpp_path).read()
    if MARK_ALIAS in src:
        print(f"{cpp_path}: dup-aliasing fix already applied, skipped")
        return
    name = "zx86_movlps_movlpd_op_en_rm"
    def_start, def_end = function_body(src, name, "fix 9")
    # the lookbehind keeps startup_/finish_ scratch-lifecycle calls out of the match
    call = re.compile(r"(?<![A-Za-z0-9_])" + re.escape(name) + r"\([^)]*\);")
    sites = [m for m in call.finditer(src) if not def_start <= m.start() <= def_end]
    # One dispatch arm per mandatory prefix, identified by the last prefix
    # comparison ahead of the call: NP and 66 are genuine, F3 and F2 aliased.
    guards = {}
    for m in sites:
        cmps = PREFIX_CMP.findall(src[m.start() - 3500 : m.start()])
        if not cmps:
            raise SystemExit(f"{cpp_path}: no prefix guard before a {name} call — fix 9 needs review")
        guards.setdefault(cmps[-1].upper(), []).append(m)
    if sorted(guards) != ["00", "66", "F2", "F3"] or any(len(v) != 1 for v in guards.values()):
        raise SystemExit(
            f"{cpp_path}: expected one {name} dispatch call per prefix 00/66/F2/F3, got "
            f"{ {k: len(v) for k, v in guards.items()} } — model layout changed, fix 9 needs review"
        )
    # back to front, so the earlier site's offsets stay valid after the first splice
    for m in sorted((guards["F3"][0], guards["F2"][0]), key=lambda m: -m.start()):
        src = src[: m.start()] + f"{MARK_ALIAS} {UNIMPL_CALL};" + src[m.end() :]
    with open(cpp_path, "w", newline="\n") as f:
        f.write(src)
    print(f"{cpp_path}: F3/F2 0F 12 now raise the unimplemented-opcode error")
...
if __name__ == "__main__":
    ...
    fix_movd_zero_extend(cpp)
    fix_dup_aliasing(cpp)
